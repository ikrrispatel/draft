...
|-------|---------------|------------|--------------|
| `vision.wedge_statement` expresses exactly one value proposition (no "and" joining two distinct props) | `WEDGE_NOT_SINGLE` | `vision.wedge_statement` | `vision` |
...
4. **`revision_requests`** — array of revision request objects. Each has: `target_agent`, `instruction`.
